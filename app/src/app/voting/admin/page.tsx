...
import { getVotesByCategory } from "@/lib/api/voting";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        for (const category of catRes.categories || []) {
          const votes = await getVotesByCategory(category);
          resultsObj[category] = votes;
        }
      ...
      {categories.map(category => (
        <div key={category} className="mb-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {(results[category] || []).sort((a, b) => b.votes - a.votes).map(({ carId, votes }) => {
